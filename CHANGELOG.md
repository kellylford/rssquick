...
- **What a load did is now actually announced.** The summary — "Loaded 380 headlines from 17 of 20 feeds; 3 failed" — was written to the status bar and then immediately overwritten by the position announcement that the load's own selection triggered, so nobody ever heard it. It now holds until you move off the first headline, at which point position takes over as before.
