...
Key bindings are registered in `SetupKeyboardNavigation()` as `InputBindings`: F5 refresh, F6 / Ctrl+Tab cycle panels, Alt+B open in browser, Alt+D make the list on screen the default. Alt+D is a binding rather than an access key so it can say in the status bar why it did nothing when the button is disabled. Left/Right expand/collapse are handled in `FeedTree_KeyDown`.
...
project lists every source file and a new one is otherwise missing from the iOS build. `ios/scripts/release-testflight.sh <build>` archives and
uploads to TestFlight. Read `ios/README.md` first: it lists where iOS deliberately differs from
